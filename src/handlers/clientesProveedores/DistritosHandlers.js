const { getAllDistrito, createDistrito, deleteDistrito, updateDistrito, searchDistritos } = require("../../controllers/clientesProveedores/distritosControllers");
...
const searchDistritosHandler = async (req,res)=>{
    let search = req.body;
    try {
        const results = await searchDistritos(search);
        res.status(201).json(results);
    } catch (error) {
        res.status(400).json({error:error.message})
    };
...
module.exports ={getDistritoHandler, createDistritoHandler, deleteDistritoHandler, updateDistritoHandler, searchDistritosHandler};
